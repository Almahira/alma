...
import { receivingDocuments, receivingItems, } from "../../../../modules/mdl_receiving/src/server/schema.js";
import { warehouseDistributions, warehouseSpoilWastes, } from "../../../../modules/mdl_warehouse/src/server/schema.js";
import { executiveTargets, executiveAllocations, executiveOwnerLedger, } from "../../../../modules/mdl_executivepanel/src/server/schema.js";
import { outlets, regions, } from "../../../../modules/mdl_organization/src/server/schema.js";
import { itemCategories, itemProducts, } from "../../../../modules/mdl_item/src/server/schema.js";
...
router.get("/financials", async (req, res) => {
    try {
        ...
        const devidenPosition = req.query.devidenPosition || "TOP_NET_SALES";
        const showTaxService = req.query.showTaxService === "true";
        // 1. QUERY MASTER DATA (OUTLETS, CATEGORIES, PRODUCTS)
        const [allOutlets, allRegions, allCategories, allProducts] = await Promise.all([
        ...
        const totalGrossSales = salesDocs.reduce((sum, d) => sum + (d.grossSales || 0), 0);
        const totalDiscount = salesDocs.reduce((sum, d) => sum + (d.discount || 0), 0);
        const totalNetSales = salesDocs.reduce((sum, d) => sum + (d.netSales || 0), 0);
        const totalTax = salesDocs.reduce((sum, d) => sum + (d.tax || 0), 0);
        const totalService = salesDocs.reduce((sum, d) => sum + (d.service || 0), 0);
        ...
        const beverageSalesPct = Number(targetConfig.beverageSalesTargetPct ?? 15) / 100;
        ...
        if (outletId)
            rcvConditions.push(eq(receivingDocuments.outletId, outletId));
        if (regionId)
            rcvConditions.push(eq(receivingDocuments.regionId, regionId));
        ...
        const dynamicPendukungMap = {};
        allCategories.forEach((cat) => {
            const catUpper = (cat.name || "").toUpperCase().trim();
            const isCogs = catUpper.includes("FOOD") ||
            ...
            const isEdr = catUpper.includes("EDR") ||
        ...
        rcvItemsList.forEach((item) => {
            ...
            const rawCatName = (categoryObj?.name ||
                (productObj?.isExpense ? "Biaya Operasional" : "FOOD")).trim();
            const catUpper = rawCatName.toUpperCase();
            const nameUpper = (productObj?.name || "").toUpperCase();
            if (catUpper.includes("PRIVE") ||
        ...
        const totalEdrFromWarehouse = distributionsList
            ...
            return (divUpper.includes("KARYAWAN") ||
        ...
        const totalSpoilLoss = spoilList.reduce((sum, sw) => sum + Number(sw.totalLossCost || 0), 0);
        const totalBelanjaDapur = foodCost + beverageCost;
        const usageBelanjaDapur = totalBelanjaDapur + totalSpoilLoss;
        const totalBelanjaPendukung = Object.values(dynamicPendukungMap).reduce((sum, c) => sum + c.amount, 0) +
            totalEmployeeMeals;
        ...
        const cogsBudgetLimit = Math.round(targetSales * (Number(targetConfig.cogsBudgetPct || 35) / 100));
        ...
        const devidenMitraDoc = ownerList.find((o) => o.category === "DEVIDEN_MITRA");
        const devidenMitraTotal = ownerList
            ...
            (Number(o.percentage || 0) > 0
                ...
                : Number(o.amount || 0)), 0);
        const priveTotal = ownerList
            .filter((o) => o.category === "PRIVE")
            .reduce((sum, o) => sum +
            (Number(o.percentage || 0) > 0
                ? Math.round(totalNetSales * (Number(o.percentage) / 100))
                : Number(o.amount || 0)), 0) + totalPriveKasirFromReceiving;
        ...
        const allocList = await db
            ...
            .where(and(eq(executiveAllocations.isActive, true), eq(executiveAllocations.month, month)));
        const alokasiUmroh = allocList
            .filter((a) => a.name.includes("UMROH"))
            .reduce((sum, a) => sum +
            (Number(a.percentage || 0) > 0
                ? Math.round(totalNetSales * (Number(a.percentage) / 100))
                : Number(a.nominal || 0)), 0) || Math.round(totalNetSales * 0.02);
        const alokasiThr = allocList
            .filter((a) => a.name.includes("THR"))
            .reduce((sum, a) => sum +
            (Number(a.percentage || 0) > 0
                ? Math.round(totalNetSales * (Number(a.percentage) / 100))
                : Number(a.nominal || 0)), 0) || Math.round(totalNetSales * 0.01);
        ...
        const payload = {
            ...
            gop: {
                grossOperatingProfit,
                gopPercentage: totalNetSales > 0
                    ? parseFloat(((grossOperatingProfit / totalNetSales) * 100).toFixed(2))
                    : 0,
            },
            ownerExpenses: {
                prive: priveTotal,
                ...
                alokasiUmroh,
                alokasiThr,
                devidenMitra: devidenPosition === "BOTTOM_OWNER" ? devidenMitraTotal : 0,
                totalOwnerExpenses,
            },
            finalProfit: {
                finalProfitOwner,
                finalProfitPercentage: totalNetSales > 0
            ...
            budgeting: {
                targetSales,
                salesAchievedPct: targetSales > 0
                    ? Math.min(100, parseFloat(((totalNetSales / targetSales) * 100).toFixed(1)))
                    : 0,
                cogsLimit: cogsBudgetLimit,
                cogsUsedPct: cogsBudgetLimit > 0
                ...
                opexUsedPct: opexBudgetLimit > 0
                    ? parseFloat(((totalBelanjaPendukung / opexBudgetLimit) * 100).toFixed(1))
                    : 0,
...
// =========================================================================
router.get("/raw-context", async (req, res) => {
    try {
        const [allRegions, allOutlets, allVendors, allReceivingDocs] = await Promise.all([
            db.select().from(regions).where(eq(regions.isActive, true)),
            db.select().from(outlets).where(eq(outlets.isActive, true)),
            ...
            db
                .select()
                .from(receivingDocuments)
                .where(and(eq(receivingDocuments.isActive, true), sql `${receivingDocuments.status} != 'CANCELLED'`)),
        ]);
        res.status(200).json({
            regions: allRegions,
            outlets: allOutlets,
