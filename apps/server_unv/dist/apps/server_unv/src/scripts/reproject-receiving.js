...
async function main() {
    ...
    const { txEventJournal } = await import("../../../../packages/db-schema/index.js");
    const { receivingHandlers } = await import("../../../../modules/mdl_receiving/src/server/event-handlers.js");
    try {
        ...
        for (const rawEvt of events) {
            const payload = typeof rawEvt.payload === "string"
            ...
            const handler = receivingHandlers[rawEvt.type];
            if (handler) {
                await db.transaction(async (tx) => {
                    try {
                        await handler(tx, eventObj);
                        reprojectedCount++;
                        console.log(`  ✓ Diproyeksikan ulang: [${rawEvt.type}] ${rawEvt.aggregateId} (v${rawEvt.aggregateVersion})`);
                    }
                    catch (err) {
                        console.warn(`  - Dilewati / Sudah ada: ${rawEvt.aggregateId} (${err.message})`);
                    }
                });
            }
        }
        console.log("====================================================");
        console.log(`  SUKSES! ${reprojectedCount} event berhasil diproyeksikan ke tabel receiving.`);
        console.log("====================================================");
    }
    catch (error) {
