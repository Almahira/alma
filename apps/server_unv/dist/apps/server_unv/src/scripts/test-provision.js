...
function pass(testName, detail) {
    console.log(`  ${c.green}✔ [PASS]${c.reset} ${c.bold}${testName}${c.reset} ${detail ? `(${detail})` : ""}`);
}
function fail(testName, reason) {
    console.log(`  ${c.red}✖ [FAIL]${c.reset} ${c.bold}${testName}${c.reset} -> ${reason}`);
}
...
===================================================================${c.reset}
  `);
    ...
    try {
        ...
        if (statusRes.ok) {
            pass("System Status Berhasil Diakses", `Virgin State: ${statusData.isVirgin}`);
        }
        else {
        ...
        if (coldRes.ok && coldData.status === "SUCCESS") {
            ...
            pass("Tenant Uji Coba Berhasil Dibuat", `Company ID: ${testCompanyId}, Mesin #1 Terdaftar`);
        }
        else {
        ...
        if (dev11Res.status === 403) {
            pass("Penolakan Kuota Berhasil!", `HTTP 403 - Pesan: "${dev11Data.error}"`);
        }
        else if (dev11Res.ok) {
            fail("Pencegahan Kuota Bocor!", `Perangkat ke-11 berhasil mendaftar padahal batas Free adalah 10. Status: ${dev11Res.status}`);
        }
        else {
            fail("Respons Tidak Terduga", `HTTP ${dev11Res.status} - ${dev11Data.error}`);
        }
        ...
        const newPremiumToken = LicenseManager.generateLicenseToken(premiumLicensePayload, masterSecretKey);
        const upgradeRes = await fetch(`${SERVER_URL}/api/provision/upgrade-license`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                companyId: testCompanyId,
        ...
        if (upgradeRes.ok && upgradeData.status === "SUCCESS") {
            pass("Upgrade Lisensi Sukses", `Tier: ${upgradeData.tier}, Kuota 50 Aktif`);
            console.log(`  ${c.magenta}↳ Mendaftarkan kembali Perangkat ke-11 pasca-upgrade...${c.reset}`);
            ...
            if (retry11Res.ok) {
                ...
                pass("Perangkat ke-11 Berhasil Terdaftar Pasca-Upgrade", `ID: ${dev11Id}`);
            }
            else {
        ...
        if (takeoverRes.ok && takeoverData.status === "SUCCESS") {
            pass("Takeover Berhasil!", `Mesin Lama (${victimDeviceId.substring(0, 16)}...) dinonaktifkan -> Mesin Baru (${replacementDeviceId.substring(0, 16)}...) aktif`);
        }
        else {
