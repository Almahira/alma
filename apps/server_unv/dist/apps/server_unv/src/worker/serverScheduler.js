...
export function setupDefaultServerTasks(uploadsDir = path.join(process.cwd(), "uploads")) {
        ...
        execute: async () => {
            ...
            for (const file of files) {
                ...
                try {
                    ...
                    console.error(`[SERVER SCHEDULER] Gagal memeriksa file ${file}:`, err);
                }
            }
            if (cleanedCount > 0) {
                console.log(`[SERVER SCHEDULER] Berhasil menghapus ${cleanedCount} file sementara yang kedaluwarsa.`);
            }
        },
    });
    ...
    globalServerScheduler.register({
        ...
        execute: async () => {
            ...
            console.log(`[SERVER HEALTH] Uptime: ${Math.floor(process.uptime())}s | Heap: ${heapUsedMb}MB | RSS: ${rssMb}MB`);
        },
    });
}
