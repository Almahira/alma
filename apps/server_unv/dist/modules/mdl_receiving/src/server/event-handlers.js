...
export const receivingHandlers = {
    ...
    RECEIVING_PAYMENT_ADDED: async (tx, event) => {
        ...
        const paymentId = p.data?.paymentId ||
            p.paymentId ||
        ...
        await tx.insert(schema.receivingPayments).values({
            ...
            paymentDate: safeDate(p.data?.paymentDate || p.timestamp || p.paymentDate),
    ...
    RECEIVING_PAYMENT_VOIDED: async (tx, event) => {
        ...
        const voidReason = event.payload.data?.voidReason ||
        ...
        if (currentDoc.length > 0) {
            const paymentStatus = newPaidAmount >= currentDoc[0].totalAmount
                ? "PAID"
                : newPaidAmount > 0
                    ? "PARTIAL"
                    : "UNPAID";
    ...
    RECEIVING_CANCELLED: async (tx, event) => {
        const cancelReason = event.payload.data?.cancelReason ||
