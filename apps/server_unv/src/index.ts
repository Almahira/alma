...
} from "./worker/serverScheduler.js";
...
const corsOptions: cors.CorsOptions = {
  origin: (origin, callback) => {
    ...
    if (
      ...
      const isPrivateIP =
        /^https?:\/\/(192\.168\.|10\.|172\.(1[6-9]|2[0-9]|3[0-1])\.|localhost)/.test(
          origin,
        );
      if (isPrivateIP) {
        callback(null, true);
...
// ============================================================
// 2. STRATEGI RATE LIMITING MEMORY STORE
// ============================================================
...
export const serverRateLimiter = (limit: number, windowMs: number) => {
  return (req: any, res: any, next: any) => {
    ...
    if (record.count > limit) {
      console.warn(
        `[RATE LIMITER] Terlalu banyak permintaan dari Device: ${clientId} (${record.count}/${limit})`,
      );
      return res.status(429).json({
        error: "TOO_MANY_REQUESTS",
        message:
          "Server sedang sibuk memproses antrean. Silakan coba beberapa saat lagi.",
...
export const serverBackpressureGuard = (req: any, res: any, next: any) => {
  if (globalActiveConnections >= MAX_CONCURRENT_SYNC) {
    console.warn(
      `[BACKPRESSURE] Server overload! Koneksi aktif: ${globalActiveConnections}/${MAX_CONCURRENT_SYNC}. Memicu pengereman klien.`,
    );
  ...
  try {
    ...
    if (deviceId && deviceId !== "UNKNOWN" && deviceId !== "SERVER") {
      ...
      if (
        ...
        console.warn(
          `[KILL SWITCH] Menolak PULL Tx dari perangkat ${deviceId} (Status: ${devCheck[0].status})`,
        );
        return res.status(403).json({
          error: "DEVICE_DEACTIVATED",
          message:
    ...
    txEventsRaw.forEach((evt) => {
      ...
      // 3. JIKA PERANGKAT ADALAH GUDANG PUSAT / REGION (Tanpa Outlet)
        ...
        const targetVendorId =
          p.reference?.supplierId || p.vendorId || p.data?.vendorId;
...
// ============================================================
app.post(
  "/api/telemetry/metrics/batch",
  serverRateLimiter(50, 60 * 1000),
  serverBackpressureGuard,
  async (req, res) => {
    try {
      ...
      const insertPayload = metrics.map((m: any) => ({
        id: m.id,
        deviceId: deviceId,
        metricName: m.name,
...
io.on("connection", async (socket) => {
  ...
  if (
    ...
    if (
      ...
      console.warn(
        `[KILL SWITCH] Memutuskan koneksi perangkat ${queryDeviceId} (${devCheck[0].status})`,
      );
    ...
    if (devCheck.length > 0) {
      ...
      console.log(
        `[SOCKET SPATIAL] Device ${dev.name} (${dev.id}) bergabung ke Room -> Company: ${dev.companyId} | Region: ${dev.regionId || "-"} | Outlet: ${dev.outletId || "-"}`,
      );
    }
    ...
    socket.on("DEVICE_HEARTBEAT", async () => {
      try {
        ...
        console.warn(
          `[HEARTBEAT] Gagal update lastSeenAt untuk ${queryDeviceId}`,
        );
      }
    });
  }
  ...
  socket.on("SYNC_UP_EVENTS", async (event, callback) => {
    ...
    try {
      ...
      if (originDeviceId && originDeviceId !== "SERVER") {
        ...
        if (!devStatus.active) {
          console.warn(
            `[SECURITY BLOCKED] Event ${event.type} DITOLAK! Perangkat ${originDeviceId} non-aktif.`,
          );
          if (typeof callback === "function") {
            return callback({
              status: "REJECTED",
