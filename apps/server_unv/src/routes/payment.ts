...
router.post("/notification", async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[MIDTRANS WEBHOOK] Notifikasi diterima untuk Order: ${orderId} [Status: ${transactionStatus}]`,
    );
    ...
    if (isPaid) {
      ...
      console.log(
        `[LICENSE GENERATED] Sukses membuat token Ed25519 untuk ${order.companyName} (${order.customerEmail}) - Kuota: ${maxOutlets} Mesin`,
      );
