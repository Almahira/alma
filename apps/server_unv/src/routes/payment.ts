...
function createMailTransporter() {
  ...
  if (!user || !pass) {
    console.warn(
      "[SMTP MAILER] Kredensial SMTP belum disetel di .env. Email akan dicetak ke log server.",
    );
    return null;
  }
...
function generateLicenseEmailHtml(params: {
  ...
  const formattedExpiry = new Date(params.validUntil).toLocaleDateString(
    "id-ID",
    {
      day: "numeric",
      month: "long",
...
async function fulfillOrderPayment(
  ...
  const maxOutlets =
    order.maxOutlets || (order.tier === "EXCLUSIVE" ? 100 : 50);

  const licensePayload: AlmaLicensePayload = {
    licenseId: `LIC_${ulid()}`,
    tier: order.tier as "PREMIUM" | "EXCLUSIVE",
  ...
  console.log(
    `[LICENSE GENERATED] Berhasil menerbitkan token Ed25519 untuk ${order.companyName} (${order.customerEmail}) - Kuota: ${maxOutlets} Mesin`,
  );
  ...
  try {
    ...
    emailStatus = "FAILED";
    console.error(
      `[SMTP MAILER ERROR] Gagal mengirim email ke ${order.customerEmail}:`,
      mailErr.message,
    );
  }
...
// =========================================================================
router.post("/create-snap", async (req: Request, res: Response) => {
  try {
    const { tier, companyName, customerName, email, phone, companyId } =
      req.body;
    ...
    const itemName = `Lisensi ${tier} (1 Thn / ${maxOutlets} Mesin)`.slice(
      0,
      50,
    );
    const targetModules = [
    ...
    console.log(
      `[MIDTRANS CREATE] Mode: ${isProduction ? "PRODUKSI" : "SANDBOX"} | Endpoint: ${midtransSnapUrl}`,
    );
    ...
    if (!snapRes.ok) {
      ...
      const errorMsg =
        (Array.isArray(snapData.error_messages)
...
router.post("/notification", async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[MIDTRANS WEBHOOK] Notifikasi diterima untuk Order: ${orderId} [Status: ${transactionStatus}, Fraud: ${fraudStatus || "N/A"}]`,
    );
