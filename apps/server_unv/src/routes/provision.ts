...
async function checkDeviceQuota(
  companyId: string,
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Data inisialisasi tidak lengkap. Perusahaan, Akun Superadmin, dan Perangkat wajib diisi.",
      });
    }
...
router.post("/login", async (req: Request, res: Response) => {
  try {
    ...
    const user = await db
      ...
      .where(
        and(
          eq(userAccounts.username, email.toLowerCase().trim()),
...
router.get("/devices-by-scope", async (req: Request, res: Response) => {
  try {
    const { companyId, regionId, outletId } = req.query as {
      companyId?: string;
      regionId?: string;
      outletId?: string;
    };
...
router.post("/device", async (req: Request, res: Response) => {
  try {
    ...
    const quotaCheck = await checkDeviceQuota(
      companyId,
      licenseKey,
      licenseTier,
    );
...
router.patch("/upgrade-license", async (req: Request, res: Response) => {
  try {
...
export const provisionRouter: Router = router;
