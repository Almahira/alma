    ...
    <form onSubmit={handleSave} className="flex flex-col h-full space-y-4">
      ...
      <div className="pt-4 flex justify-end gap-3 border-t border-(--border-color) shrink-0">
        <button
          type="button"
          onClick={onClose}
          className="px-5 py-2 text-xs font-bold text-(--text-secondary) hover:bg-(--surface-hover) rounded-lg transition"
        >
          BATAL
        </button>
        <button
          type="submit"
          className="px-5 py-2 text-xs font-black text-white bg-orange-500 rounded-lg hover:bg-orange-600 transition shadow-[0_4px_10px_rgba(249,115,22,0.3)]"
        >
          SIMPAN VENDOR
        </button>
      </div>
        ...
        doc.fileName.match(/\.(jpg|jpeg|png)$/i) ? (
          ...
          <div className="bg-(--bg-card) w-full h-full shadow-md rounded flex flex-col items-center justify-center border-2 border-dashed border-(--border-color)">
            ...
            <a
              ...
              className="mt-5 px-6 py-2 bg-orange-500 text-white rounded font-bold hover:bg-orange-600 transition"
            >
              Unduh File
            </a>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {viewStatus === "AKTIF" && (
              <div className="relative" ref={actionMenuRef}>
                ...
                >
                  <Layers className="w-4 h-4 text-orange-500" /> AKSI & DOKUMEN
                  <ChevronDown
