...
export function SpoilWastePage() {
  ...
  const availableRecipeOptions = useMemo(() => {
    return recipes
      .filter((r: any) => {
        ...
        if (localCompanyId && r.companyId && r.companyId !== localCompanyId)
          return false;

        if (localOutletId) {
          return !r.outletId || r.outletId === localOutletId;
        } else if (filterOutletId) {
          return !r.outletId || r.outletId === filterOutletId;
        }
        return true;
      })
      .map((r) => ({
  ...
  }, [recipes, localCompanyId, localOutletId, filterOutletId]);
